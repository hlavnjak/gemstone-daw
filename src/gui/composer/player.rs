...
impl CompositionPlayer {
    ...
    pub fn start(plans: Vec<RowPlan>) -> Result<Self> {
        ...
        for plan in plans {
            ...
            let _ = inst.initialize_audio(sample_rate, max_block as i32);
        ...
        let stream = device.build_output_stream(
            ...
            move |out: &mut [f32], _: &cpal::OutputCallbackInfo| {
                    ...
                    // Events due in this block, offset to their sample in it.
                    {
                        ...
                        while let Some(&(at, pitch, on)) = voice.schedule.get(voice.cursor) {
                            ...
                            if let Some(mut ev) = midi_to_vst3_event([status, pitch, velocity]) {
                                // Events already due (a late start, or two in the
                                // same block) land on the block's first sample.
                                ev.sampleOffset =
                ...
                // The mix is a sum of independent instruments, so clamp rather
                // than let the device wrap on a loud chord.
